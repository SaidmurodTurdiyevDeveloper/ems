...
object DateUtil {
    fun calculateTimePercent(startTime: String, endTime: String): Float {
        try {
    ...
    fun getUntilTimeCount(context: Context, lastDate: String): String {
        try {
            ...
            if (nowYear > year) return context.getString(commonR.string.until_year, nowYear - year)
            if (nowMonth > month) return context.getString(commonR.string.until_month, nowMonth - month)
            if (nowDay > day) return context.getString(commonR.string.until_day, nowDay - day)
            if (nowHour > hour) return context.getString(commonR.string.until_hour, nowHour - hour)
            if (nowMinute > minute) return context.getString(commonR.string.until_minute, nowMinute - minute)
            return ""
        } catch (_: Exception) {
            return ""
        }
